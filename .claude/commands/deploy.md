...
2. Run `sf project deploy preview --source-dir force-app` to show what would be deployed
...
For **validation only (dry run)** — tests run, nothing actually deploys:
```
...
- **Code coverage < 75%**: Need to add/fix test classes. Run `sf apex run test --synchronous --result-format human --code-coverage` to see per-class coverage.
